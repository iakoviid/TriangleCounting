...
__global__ void findColStart(int *dJ, int len, int *col) {
...
__global__ void computeRow1(int* dI,int* dJ,int nz,int* col,int* out, int N) {
int i=blockIdx.x;
while(i<N){
...
int main(int argc, char *argv[])
{
    ...
    InitColStart<<<ceil(N/threadsPerBlock), threadsPerBlock>>>(N,col);
    findColStart<<<ceil(nz/threadsPerBlock), threadsPerBlock>>>(dJ,nz,col);
    //colLengths<<<ceil(N/threadsPerBlock), threadsPerBlock>>>(N,col);


    computeRow1<<<ceil(N/64), 64,threadsPerBlock>>>(dI,dJ,nz,col,out,N);
